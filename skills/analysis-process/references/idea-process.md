...
- Assume the developer who is going to implement the feature is experienced and highly skilled in the project's primary language, but has zero context for our codebase and knows almost nothing about our problem domain. Basically — a first-time contributor with strong programming experience.
...
- **Always include a final verification task** that depends on all other tasks — it should invoke `testing-process` to run the full test suite, `documentation-process` to update any relevant docs, and a `solid-code-review` with project's language input to review the code
