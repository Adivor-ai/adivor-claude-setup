...
- Prompt user for code-review; if user responds 'yes' — use the `solid-code-review` skill for a structured review. For additional perspective, use the `cove` skill (Chain-of-Verification) to independently verify critical logic, or spawn a sub-agent with the Agent tool to perform a parallel review focused on a specific concern (e.g., security, performance).
...
### Plan becomes outdated mid-implementation
**Issue**: Design docs or requirements have changed, making the current implementation plan obsolete.
**Solution**: Re-read the design docs and discuss with the user to understand what changed. Stop and ask for guidance before continuing with outdated instructions.

### Task dependencies not met
**Issue**: A task requires another task to be completed first, but the dependency isn't done.
**Solution**: Verify prerequisite tasks are marked as `done` in `tasks.md`. Don't proceed until blockers are cleared. Ask the user to clarify the dependency order.

### Blocked by unclear instructions
**Issue**: A task instruction is ambiguous, contains missing context, or contradicts other docs.
**Solution**: Stop and ask for clarification rather than guessing. Quote the unclear instruction and ask what the expected outcome should be.

### Tests fail after implementation
**Issue**: Tests break or fail after completing a task, blocking progression to the next task.
**Solution**: Use the `testing-process` skill to debug and fix the tests before marking the task done. Don't move forward with failing tests.
