...
See [cove-process.md](./references/cove-process.md) for the standard workflow.
...
See [cove-isolated.md](./references/cove-isolated.md) for the isolated workflow.
...
See [cove-process.md](./references/cove-process.md) for the standard workflow, or [cove-isolated.md](./references/cove-isolated.md) for the isolated sub-agent workflow.
...
### Verification questions too similar to original
**Issue**: Verification questions rephrase the original instead of testing it independently, defeating the purpose of verification.
**Solution**: Rewrite verification questions from first principles without referencing the original answer. Ask about components, edge cases, and assumptions separately.

### CoVe adds too much latency
**Issue**: Verification doubles or triples response time, making it impractical for real-time use.
**Solution**: Use standard mode (`/cove`) instead of isolated mode for faster turnaround. Reserve isolated mode for high-stakes accuracy where latency cost is justified.

### Verification contradicts original answer
**Issue**: Verification questions reveal the original answer was incorrect, but discrepancy is unclear.
**Solution**: Dig deeper into both the original answer and verification findings. Identify the specific error (assumption, calculation, logic) and revise with clear reasoning for the correction.

### Sub-agent failures in isolated mode
**Issue**: Sub-agents encounter issues spawning or returning results in `/cove-isolated`.
**Solution**: Check agent availability and task system status. Fall back to standard mode (`/cove`) if sub-agents are unavailable. Use `--haiku` flag for faster, cheaper verification if cost is a concern.
