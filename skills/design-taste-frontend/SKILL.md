...
description: >-
  Web de marketing a partir de un brief de marca: landing pages, portfolios, páginas about y
  rediseños de sitios de marketing. Infiere la dirección del brief e instala un design system real
  (Material 3, Fluent, Carbon, Polaris) cuando encaja. Triggers: «haz una landing», «diseña el
  sitio», «rediseña la web», 'build a landing page'. NO usar para dashboards, UI densa de
  producto, tablas ni formularios multi-paso: eso es impeccable. NO usar para aplicar un estilo
  visual: encadénalo después.
---
...
Esta skill se recortó a dos cláusulas "NO usar" en su `description` para que dispare limpio.
Las fronteras completas quedan documentadas aquí (ver también la sección 13, OUT OF SCOPE):

| No uses design-taste-frontend para | Usa en su lugar |
|---|---|
| Dashboards, UI densa de producto, paneles de admin | `impeccable` |
| Tablas de datos, formularios multi-paso, wizards | `impeccable` |
| Un componente aislado, sin brief de marca | `frontend-designer` |
| Aplicar una estética concreta | `minimalist-ui` · `industrial-brutalist-ui` · `high-end-visual-design` |
| Editores de código, móvil nativo, colaboración en tiempo real | fuera del alcance de esta skill |
| Reproducir el comportamiento exacto de v1 | `design-taste-frontend-v1` |
